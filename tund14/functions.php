...
//Kõigi valideeritud sõnumite lugemine valideerija kaupa
  ...
  //   SELECT COUNT(*) FROM vpphotos2 WHERE deleted IS null ...
	...
	$stmt = $mysqli->prepare("SELECT id, title, content FROM vpnews WHERE expire<=? AND deleted IS NULL");
	echo $mysqli->error;
	$stmt->bind_param("s", $expire);
	$stmt->bind_result($idFromDb, $titleFromDb, $contentFromDb);
	$stmt->execute();
	$stmt->fetch();
	if(empty($notice)){
		$notice = "<p>Kahjuks uudiseid pole!</p>";
	}
	$stmt->close();
	$mysqli->close();
	return $notice;  
	...
	function addnews($title, $content, $expire){
	$notice = "";
	//uudise lisanud kasutaja id, uudise pealkiri, sisu ja aegumiskuupäev.
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	...
	if($stmt->execute()){
	  $notice = "Uudis on postitatud!";
	} else {
      echo "Uudise lisamisel tekkis tehniline viga: " .$stmt->error;
	}
    $stmt->close();
	$mysqli->close();
    return $notice;
  ...
  function readAllPublicPictureThumbsPage($page, $limit){
	...
	$stmt->bind_result($photoidFromDb, $firstnameFromDb, $lastnameFromDB, $filenameFromDb, $alttextFromDb, $ratingFromDb);
	$stmt->execute();
	while($stmt->fetch()){
	/*<div class="thumbGallery">
		...
		$html .= '<img src="' .$GLOBALS["thumbDir"] .$filenameFromDb .'" alt="'.$alttextFromDb .'" data-fn="' . $filenameFromDb.'" data-id="' .$photoidFromDb .'">' ."\n";
		$html .= "<p>" .$firstnameFromDb ." " .$lastnameFromDB ."</p> \n";
		$html .= '<p id="score' .$photoidFromDb .'">';
		if(empty($ratingFromDb)){
			$html .="Hinne puudub";
		} else{
			$html .= "Hinne: " .$ratingFromDb ."</p> \n";
		}
		$html .= "</div> \n";
  ...
  /* function readAllPublicPictureThumbsPage($page, $limit){
	...
	$stmt->execute();
	while($stmt->fetch()){
		$html .= '<img src="' .$GLOBALS["thumbDir"] .$filenameFromDb .'" alt="'.$altFromDb .'" data-fn="' . $filenameFromDb.'" data-id="' .$idFromDb .'">' ."\n";
	}
	if(empty($html)){
		$html = "<p>Kahjuks pilte pole!</p>";
	}
