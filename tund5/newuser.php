<?php
  ...
  if(!empty($_POST["birthDay"]) and !empty($_POST["birthMonth"]) and !empty($_POST["birthYear"])){
	  // kontrollime kas see kuup. eksisteerib/ kolm täisarvu= kuu, päev, aasta
  ...
  <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	...
	<label>Sünnipäev: </label>
	  <?php
		...
		echo '<option value="" selected disabled>Päev</option>' ."\n"; //disabled tähendab, et see valik ei ole valitav
		for ($i = 1; $i < 32; $i ++){
			echo '<option value="' .$i .'"';
			if ($i == $birthDay){
				echo " selected ";
	  ...
	  <br> 
	  <label>Sugu:</label><br>
	  <input name="gender" type="radio" value="2" <?php if($gender == 2){ echo "checked";}?>><label>Naine</label> <!-- radioga vormi sees saad valida punktida ühe valiku v -->
	  <br>
